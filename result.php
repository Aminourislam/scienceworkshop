...
<body>
    <div class="table-container">
        ...
        <table>
            ...
            <tbody>
                <?php
                $query = $con->prepare("SELECT user_name, roll, total_mark FROM users ORDER BY total_mark DESC LIMIT 10");
                $query->execute();
                $result = $query->get_result();

                while($row = $result->fetch_assoc()) {
                    echo "<tr><td>{$row['user_name']}</td><td>{$row['roll']}</td><td>{$row['total_mark']}</td></tr>"; 
                }
                ?>
            </tbody>
        </table><br>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query = $con->prepare("SELECT user_name, roll, college, class, mcq_mark, written_mark, total_mark FROM users WHERE user_name=? AND roll=? AND examname=?");
